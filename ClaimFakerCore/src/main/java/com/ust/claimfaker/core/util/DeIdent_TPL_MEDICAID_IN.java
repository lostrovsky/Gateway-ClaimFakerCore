...
public class DeIdent_TPL_MEDICAID_IN {

	public static void main(String[] args) throws FileNotFoundException {
		DHSTPLmodel2 thisDHSTPL2 = new DHSTPLmodel2();
		...
		while (scanner.hasNext()) {
			...
			outputFakery(thisDHSTPL, OutputFileName);
		...
		/*
		 * } finally { if (scanner != null && scanner.hasNext()) { scanner.close(); int
		 * yy=1; } }
		 */
	...
	private static void outputFakery(DHSTPLmodel thisDHSTPL, String fn) {
		...
		try {
			...
			printWriter.println(thisDHSTPL.CISRID + thisDHSTPL.MCOCode + thisDHSTPL.TypeofInsurance
					+ thisDHSTPL.CarrierName + thisDHSTPL.CarrierStreet + thisDHSTPL.CarrierCity
					+ thisDHSTPL.CarrierState + thisDHSTPL.CarrierZip + thisDHSTPL.CarrierZipExt
					+ thisDHSTPL.PolicyNumber + thisDHSTPL.CoverageBegin + thisDHSTPL.CoverageEnd
					+ thisDHSTPL.PolicyholderLastName + thisDHSTPL.PolicyholderFirstName + thisDHSTPL.PolicyholderStreet
					+ thisDHSTPL.PolicyholderCity + thisDHSTPL.PolicyholderState + thisDHSTPL.PolicyholderZip
					+ thisDHSTPL.PolicyholderZipExt + thisDHSTPL.PolicyholderSSN + thisDHSTPL.EmployerName
					+ thisDHSTPL.EmployerStreet + thisDHSTPL.EmployerCity + thisDHSTPL.EmployerState
					+ thisDHSTPL.EmployerZip + thisDHSTPL.EmployerZipExt + thisDHSTPL.EmployerTelephone
	...
	public static DHSTPLmodel getHMSTLPrec(StringBuilder sb, Faker faker) {
		...
		for (String each : wordsArray) {
			...
			if (matcher.matches())
				ft = "numeric" + field.length();
			else
				ft = "alpha" + field.length();
			...
			if (sb.substring(14, 28).trim().length() > 0) {
				field = String.format("%-14s",
						DbFakeHPP834EDIchp.findFake("lastName", "String", sb.substring(14, 28).toUpperCase(), faker))
						.substring(0, 14);
				thisDHSTPL.setLastName(field);
			} else
				thisDHSTPL.setLastName(sb.substring(14, 28));

			if (sb.substring(28, 39).trim().length() > 0) {

				field = String.format("%-11s",
						DbFakeHPP834EDIchp.findFake("firstName", "String", sb.substring(28, 39).toUpperCase(), faker))
						.substring(0, 11);
				thisDHSTPL.setFirstName(field);
			} else
				thisDHSTPL.setFirstName(sb.substring(28, 39));

			if (sb.substring(39, 40).trim().length() > 0) {

				field = DbFakeHPP834EDIchp.findFake("middleInitial", "String", sb.substring(39, 40).toUpperCase(),
						faker);
				thisDHSTPL.setMiddleInitial(field);
			} else
				thisDHSTPL.setMiddleInitial(sb.substring(39, 40));
			...
			if (sb.substring(40, 49).trim().length() > 0) {
				...
				thisDHSTPL.setSSN(sb.substring(40, 49).toUpperCase());
			...
			if (sb.substring(137, 157).trim().length() > 0) {
				...
				if (matcher.matches())
					...
					ft = "alpha" + field.trim().length();

				field = String.format("%-20s",
						DbFakeHPP834EDIchp.findFake(ft, "String", sb.substring(137, 157).trim().toUpperCase(), faker))
						.substring(0, 20);
				// System.out.println("LL " + field.length());
				thisDHSTPL.setPolicyNumber(field);
			} else
				thisDHSTPL.setPolicyNumber(sb.substring(137, 157));
			...
			if (sb.substring(193, 207).trim().length() > 0) {
				...
				field = String.format("%-14s",
						DbFakeHPP834EDIchp.findFake("lastName", "String", sb.substring(193, 207).toUpperCase(), faker))
						.substring(0, 14);
				thisDHSTPL.setPolicyholderLastName(field);
			} else
				thisDHSTPL.setPolicyholderLastName(sb.substring(193, 207));

			if (sb.substring(207, 218).trim().length() > 0) {

				field = String.format("%-11s",
						DbFakeHPP834EDIchp.findFake("firstName", "String", sb.substring(207, 218).toUpperCase(), faker))
						.substring(0, 11);
				thisDHSTPL.setPolicyholderFirstName(field);
			} else
				thisDHSTPL.setPolicyholderFirstName(sb.substring(207, 218));

			if (sb.substring(218, 219).trim().length() > 0) {
				@SuppressWarnings("unused")
				String thisone = sb.substring(218, 219);
				field = DbFakeHPP834EDIchp.findFake("middleInitial", "String", sb.substring(218, 219).toUpperCase(),
						faker);
				thisDHSTPL.setPolicyHolderMiddleInitial(field);
			} else
				thisDHSTPL.setPolicyHolderMiddleInitial(sb.substring(218, 219));

			if (sb.substring(219, 245).trim().length() > 0) {

				field = String.format("%-26s", DbFakeHPP834EDIchp.findFake("addressLine1", "String",
						sb.substring(219, 245).toUpperCase(), faker)).substring(0, 26);
				thisDHSTPL.setPolicyholderStreet(field);
			} else
				thisDHSTPL.setPolicyholderStreet(sb.substring(219, 245));

			// field = String.format("%-16s", DbFakeHPP834EDIchp.findFake("City", "String",
			// sb.substring(245, 261).toUpperCase(),faker)).substring(0,16);
			...
			if (sb.substring(272, 281).trim().length() > 0) {
				...
				thisDHSTPL.setPolicyholderSSN(sb.substring(272, 281));
